...
def test_updatable_progress():
    ...
    progress2 = display.create_updatable_progress("gpt-5.3-codex")
    progress2.start()
    time.sleep(2)
...
def test_long_context_transition():
    ...
    progress = display.create_updatable_progress("kimi-k3")  # 실제 사용한 모델명
    progress.start()
    ...
    try:
        ...
        multiturn_progress = display.create_updatable_progress("kimi-k3")
...
def test_cli_exact_reproduction():
    ...
    # CLI에서와 동일한 방식
    display = ReviewDisplay()
    progress = display.create_updatable_progress("kimi-k3")
    progress.start()
