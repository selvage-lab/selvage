"""OptimizedContextExtractor Java 테스트 케이스."""
...
class TestBasicFunctionExtraction:
    ...
    def sample_file_path(self) -> Path:
        ...
        return Path(__file__).parent / "language_samples" / "java" / "SampleCalculator.java"

    @pytest.fixture
    def extractor(self) -> OptimizedContextExtractor:
        """Java용 OptimizedContextExtractor 인스턴스를 반환합니다."""
        return OptimizedContextExtractor("java")
    ...
    def test_complex_method(
        ...
        """복잡한 메서드 추출 테스트."""
        changed_ranges = [LineRange(75, 78), LineRange(120, 125)]  # multiplyAndFormat 메서드
        contexts = extractor.extract_contexts(sample_file_path, changed_ranges)

        assert len(contexts) >= 1
        all_context = "\n".join(contexts)
        # 선언부 검증
        assert "public Map<String, Object> multiplyAndFormat(List<Integer> numbers)" in all_context
        # 내부 코드 블록 검증 (전체 메서드 추출 확인)
    ...
    def test_nested_inner_function(
        ...
        all_context = "\n".join(contexts)
        # 선언부 검증
        assert "class ProductCalculator" in all_context or "class RecursiveMultiplier" in all_context
    ...
    def test_factory_function(
        ...
        all_context = "\n".join(contexts)
        # 선언부 검증
        assert "public static SampleCalculator advancedCalculatorFactory(String mode)" in all_context
        # 내부 코드 블록 검증 (전체 함수 추출 확인)
...
class TestMultiRangeExtraction:
    ...
    def test_three_cross_classes(
        ...
        changed_ranges = [LineRange(129, 210)]  # calculateCircleArea ~ CalculatorFactory
        contexts = extractor.extract_contexts(sample_file_path, changed_ranges)
