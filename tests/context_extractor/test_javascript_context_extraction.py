"""OptimizedContextExtractor JavaScript 테스트 케이스."""
...
class TestBasicFunctionExtraction:
    ...
    def sample_file_path(self) -> Path:
        ...
        return Path(__file__).parent / "language_samples" / "javascript" / "SampleCalculator.js"

    @pytest.fixture
    def extractor(self) -> OptimizedContextExtractor:
        """JavaScript용 OptimizedContextExtractor 인스턴스를 반환합니다."""
        return OptimizedContextExtractor("javascript")
    ...
    def test_nested_inner_function(
        ...
        assert len(contexts) >= 1
        all_context = "\n".join(contexts)
        assert "function multiplyRecursive" in all_context or "multiplyRecursive" in all_context
...
class TestMultiRangeExtraction:
    ...
    def test_three_cross_functions(
        ...
        """3개 함수에 걸친 영역 추출 테스트."""
        changed_ranges = [LineRange(104, 165)]  # calculateCircleArea ~ advancedCalculatorFactory
        contexts = extractor.extract_contexts(sample_file_path, changed_ranges)
