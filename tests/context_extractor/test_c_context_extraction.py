...
class TestBasicFunctionExtraction:
    ...
    def extractor(self) -> OptimizedContextExtractor:
        """C용 OptimizedContextExtractor 인스턴스를 반환합니다."""
        return OptimizedContextExtractor("c")
...
class TestMultiRangeExtraction:
    ...
    def test_three_cross_functions(
        ...
        """3개 함수에 걸친 영역 추출 테스트."""
        changed_ranges = [LineRange(137, 209)]  # calculate_circle_area ~ advanced_calculator_factory
        contexts = extractor.extract_contexts(sample_file_path, changed_ranges)
...
class TestComplexScenarios:
    ...
    def test_entire_class_extraction(
        ...
        assert len(contexts) >= 1
        all_context = "\n".join(contexts)
        assert "struct SampleCalculator" in all_context or "SampleCalculator" in all_context
        assert "create_sample_calculator" in all_context
        assert "add_numbers" in all_context
