"""OptimizedContextExtractor TypeScript 테스트 케이스."""
...
class TestBasicFunctionExtraction:
    ...
    def sample_file_path(self) -> Path:
        ...
        return Path(__file__).parent / "language_samples" / "typescript" / "SampleCalculator.ts"

    @pytest.fixture
    def extractor(self) -> OptimizedContextExtractor:
        """TypeScript용 OptimizedContextExtractor 인스턴스를 반환합니다."""
        return OptimizedContextExtractor("typescript")
    ...
    def test_constructor_method(
        ...
        assert "this.value = initialValue" in all_context or "this.value: number" in all_context
    ...
    def test_external_function(
        ...
        assert len(contexts) >= 1
        all_context = "\n".join(contexts)
        assert "function helperFunction" in all_context or "helperFunction" in all_context
...
class TestMultiRangeExtraction:
    ...
    def test_three_cross_functions(
        ...
        """3개 함수에 걸친 영역 추출 테스트."""
        changed_ranges = [LineRange(119, 180)]  # calculateCircleArea ~ advancedCalculatorFactory
        contexts = extractor.extract_contexts(sample_file_path, changed_ranges)
