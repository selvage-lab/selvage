...
class TestReviewSynthesizerLLMIntegration:
    ...
    def test_provider_specific_params_openrouter(self) -> None:
        ...
        assert params["response_format"]["json_schema"]["name"] == "structured_synthesis_response"
...
class TestReviewSynthesizerEndToEndMock:
    ...
    def sample_multiturn_data(self) -> dict[str, Any]:
        ...
        test_data_path = Path(__file__).parent.parent / "data" / "sample_multiturn_results.json"
    ...
    def test_end_to_end_mock_openai_success(
        ...
        mock_structured_response = StructuredSynthesisResponse(**mock_successful_llm_response)
        mock_raw_response = Mock()
        mock_instructor_client.chat.completions.create_with_completion.return_value = (
            mock_structured_response, mock_raw_response
        )
    ...
    def test_end_to_end_mock_llm_failure_with_retry_then_fallback(
        ...
        mock_instructor_client = Mock()
        mock_create_client.return_value = mock_instructor_client
        mock_instructor_client.chat.completions.create_with_completion.side_effect = Exception("API 호출 실패")
        ...
        # 3회 재시도 확인
        assert mock_instructor_client.chat.completions.create_with_completion.call_count == 3
    ...
    def test_combine_recommendations_simple_method(
        ...
        recommendations = synthesizer._combine_recommendations_simple(integration_review_results)
    ...
    def test_synthesize_summary_only_method(
        ...
        original_summaries = [r.review_response.summary for r in integration_review_results]
        longest_summary = max(original_summaries, key=len)
        assert summary == longest_summary

    def test_get_language_instruction_korean(self) -> None:
        """한국어 언어 지시사항 생성 테스트"""
        synthesizer = ReviewSynthesizer("test-model")
        
        # Korean 설정들 테스트
        korean_variations = ["korean", "Korean", "ko", "한국어"]
        
        for lang in korean_variations:
            instruction = synthesizer._get_language_instruction(lang)
            assert "Korean language" in instruction
            assert "한국어로 응답하세요" in instruction

    def test_get_language_instruction_english(self) -> None:
        """영어 언어 지시사항 생성 테스트"""
        synthesizer = ReviewSynthesizer("test-model")
        
        # English 및 기타 언어들 테스트
        english_variations = ["english", "English", "en", "other"]
        
        for lang in english_variations:
            instruction = synthesizer._get_language_instruction(lang)
            assert "English language" in instruction
            assert "한국어로 응답하세요" not in instruction

    @patch("selvage.src.multiturn.review_synthesizer.get_default_language")
    def test_get_synthesis_system_prompt_with_korean_setting(self, mock_get_language: Mock) -> None:
        """한국어 설정 시 시스템 프롬프트에 언어 지시사항 포함 테스트"""
        # Given: 한국어 설정
        ...
        # Then: 한국어 언어 지시사항이 포함되어야 함
        assert "한국어로 응답하세요" in prompt
        assert "Korean language" in prompt
        assert "expert code review synthesis specialist" in prompt  # 기본 프롬프트도 포함

    @patch("selvage.src.multiturn.review_synthesizer.get_default_language")
    def test_get_synthesis_system_prompt_with_english_setting(self, mock_get_language: Mock) -> None:
        """영어 설정 시 시스템 프롬프트에 언어 지시사항 포함 테스트"""
        # Given: 영어 설정
