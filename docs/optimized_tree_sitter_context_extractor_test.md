...
class TestDiffResultIntegration:
    ...
    def test_end_to_end_workflow(self, tmp_code_directory):
        ...
        # 3. LineRange 변환
        line_ranges_results = diff_result.to_line_ranges_results()

        target_result = next((r for r in line_ranges_results if r.file_name == filename), None)
        assert target_result is not None
        line_ranges = target_result.line_ranges
    ...
    def test_multiple_files_diff(self, tmp_code_directory):
        ...
        all_contexts = {}
        for line_range_result in line_ranges_results:
            filename = line_range_result.file_name
            if filename in extractors:
                file_path = tmp_code_directory / filename
                if file_path.exists():
                    contexts = extractors[filename].extract_contexts(
                        file_path,
                        line_range_result.line_ranges
                    )
    ...
    def test_diff_with_additions_deletions(self, tmp_code_directory):
        ...
        target_result = next((r for r in line_ranges_results if r.file_name == filename), None)
        assert target_result is not None
        contexts = extractor.extract_contexts(file_path, target_result.line_ranges)
